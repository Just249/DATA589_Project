...
bas_bc_data <- occ_data(scientificName = species,
                           hasCoordinate = TRUE,
                           country = "CA",
                           stateProvince = "British Columbia",
                           limit = 2000) #* 102 illegal points stored in attr(,“rejects”) ***

# class(bas_bc_data) #gbif_data 
bas_bc_data <- bas_bc_data$data #gbif_data to data.frame
## head(bas_bc_data) #contain "Ascomycota" data only in BC
```
...
```{r include=FALSE}
bas_bc_data <- apply(bas_bc_data,2,as.character)
...
write.csv(bas_bc_data, "data/bas_bc_data.csv", row.names = FALSE)
bas_bc_data <- read.csv("data/bas_bc_data.csv")
...
As the dataset contains too much information, including many detailed timestamps, internal key/identifiers information and dataset/records identifiers which should not be valuable in our analysis and complicate our subsequent analysis, we have performed preliminary cleaning procedure and performed attributes selection, in order to only retain those few potentially useful attributes, speeding up our subsequent analysis and making it more focused.  The cleaned list is shown below.

```{r}

# View(data.frame(names(bas_bc_data)))
cleaned_bas_bc <- bas_bc_data[ , c("decimalLongitude", "decimalLatitude", "order", "family", "genus", "species", "genericName", "specificEpithet", "coordinateUncertaintyInMeters", "stateProvince", 
...
# convert cleaned_bas_bc to SpatialPointsDataFrame-class
...
names(cleaned_bas_bc)
sort(unique(cleaned_bas_bc$individualCount))  # notice if some points correspond to zero abundance
sort(unique(cleaned_bas_bc$occurrenceStatus))  # check for different indications of "absent", which could be in different languages
absence_rows <- which(cleaned_bas_bc$individualCount == 0 | cleaned_bas_bc$occurrenceStatus %in% c("absent", "Absent", "ABSENT", "ausente", "Ausente", "AUSENTE"))
length(absence_rows)
if (length(absence_rows) > 0) {
  cleaned_bas_bc <- cleaned_bas_bc[-absence_rows, ]
}

library(sp)
sp_bas_bc <- SpatialPointsDataFrame(coords = cleaned_bas_bc[, c("decimalLongitude", "decimalLatitude")],
                                         data = cleaned_bas_bc)


proj4string(sp_bas_bc) <- CRS("+proj=longlat +datum=WGS84")

# install.packages("rgdal")
library(rgdal)

#project basemap to match spu shapefile
tran_sp_bas_bc <- spTransform(sp_bas_bc, CRS("+proj=aea +lat_0=45 +lon_0=-126 +lat_1=50 +lat_2=58.5 +x_0=1000000 +y_0=0 +datum=NAD83 +units=m +no_defs")) 
summary(tran_sp_bas_bc)

#update to spTransformed Longitude & Latitude 
cleaned_bas_bc$decimalLongitude <- tran_sp_bas_bc@coords[, 1]
cleaned_bas_bc$decimalLatitude <- tran_sp_bas_bc@coords[, 2]

# save the data as CSV
write.csv(cleaned_bas_bc, "data/clean_sp_bas_bc", row.names = FALSE)
cleaned_bas_bc <- read.csv("data/clean_sp_bas_bc")
...
bas_data_ppp <- ppp(x = cleaned_bas_bc$decimalLongitude,
                     y = cleaned_bas_bc$decimalLatitude,
                     window = BC_win)

#length(bas_data_ppp)
#anyDuplicated(bas_data_ppp)
bas_data_ppp
```
...
# perspPoints(bas_data_ppp, Z = BC$Elevation, M = fig, pch = 16, cex = 0.7, col="black")
```
...
#points(bas_data_ppp, pch = 16, cex = 0.35, col="green")
```
```{r}
cut <- cut(BC$Elevation,5,
labels = c("low","low-medium","medium","medium-high","high"))
table(cut[bas_data_ppp]) #most in low elevation
...
```{r}
# nn_dist <- nndist(bas_data_ppp)
# marks(bas_data_ppp) <- nn_dist
# plot(bas_data_ppp, main = "Ascomycota Distance",which.marks = "Dist", pch = 16)
# BC$Elevation[bas_data_ppp]
# install.packages("kdensity")
library("kdensity")
asc_density <- density(BC$Elevation[bas_data_ppp])
...
```{r}
intensity(bas_data_ppp)
```

#### 2.3.5.b Homogeneity Studies, Quadrat Test and Hotspot Analysis

Note that the intensity is a very small number.  This is consistent with our plots above.  There are not many points in the whole BC windows.  Overwhelming majority of the points are sparesely located in the BC.  Let's verify this with the Quadrat Count Plot.


```{r}
Q <- quadratcount(bas_data_ppp,
                  nx = 4,
                  ny = 4)
...
```{r}
R <- bw.ppl(bas_data_ppp)
LR <- scanLRTS(bas_data_ppp,r=R)

#plot(LR,"Likelihood Ratio Test")
#plot(bas_data_ppp$window,"Likelihood Ratio Test")
...
plot(bas_data_ppp$window,add=T)
```
...
V <- tess(image=Zcut)
quadratcount(bas_data_ppp,tess=V)
```
```{r echo=FALSE,cache=TRUE}
rho <- rhohat(bas_data_ppp,elev)
plot(rho,xlim=c(-200,1000))
```


#### 2.3.6.b Covariate Variable : Forest

Then, let's the check Forest as covariate.
...
```{r echo=FALSE, cache=TRUE}
rho <- rhohat(bas_data_ppp,forest)
plot(rho)
```
...
Then, we look into "Distance to Water".
...
```
```{r echo=FALSE, cache=TRUE}
rho <- rhohat(bas_data_ppp,dist)
plot(rho)
```


#### 2.3.6.d Covariates Section Conclusion

The observation is made from the above plots : 
...
#### 2.3.7.a Ripley's K-function  ??? why K goes down???
...
```{r cache=TRUE}
lambda_asc <- density(bas_data_ppp, bw.ppl)
Kinhom_asc <- Kinhom(bas_data_ppp, lambda_asc)

#Estimate a strictly positive density
lambda_asc_pos <- density(bas_data_ppp,
                          sigma=bw.ppl,
                          positive=TRUE)

#Simulation envelope (with points drawn from the estimated intensity)
E_asc_inhom <- envelope(bas_data_ppp,
                        Kinhom,
...
```{r}
#Simulation envelope (with points drawn from the estimated intensity)
pcf_asc_inhom <- envelope(bas_data_ppp,
                          pcfinhom,
                          simulate = expression(rpoispp(lambda_asc_pos)),
                          rank = 1,
                          nsim = 19)
...
In this section, we will proceed with model fitting and selection, in order to achieve a reasonably effective model, while following the rule of parisomny as much as possible.  To do this, we will try to fit model with different polynomial degree terms and then assess for the performance gains versus the cost of the extra complexity.

```{r}
fit <- ppm(bas_data_ppp ~ Elevation + Dist_Water + Forest, data = BC)
fit
```
...
```{r}
fit <- ppm(bas_data_ppp ~ Elevation + I(Elevation^2) + Dist_Water + I(Dist_Water^2), data = BC)
fit
```
...
```{r}
fit_simple <- ppm(bas_data_ppp ~ Elevation + Forest, data = BC)
fit_simple
fit <- ppm(bas_data_ppp ~ Elevation + I(Elevation^2) + Forest + I(Forest^2), data = BC)
fit
```
...
#Fit the PPP model
fit_smooth <- ppm(bas_data_ppp ~ bs(Elevation,7) + bs(Forest, 3), data = BC, use.gam = TRUE)
