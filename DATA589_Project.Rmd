...
In this section, we will proceed with model fitting and selection, in order to achieve a reasonably effective model, while following the rule of parisomny as much as possible.  We will try to start with the simplest linear models.  Then we will adjust the model acccording to the data behaviour and increase the model complexity if situation warranted.

Lurking Variable plot could give us some insight on whether or not we should include the co variate that were missing from our original model. After know some potential covariates that could improve our model, Partial Residual Plot can then help us to understand if the added covariate have a linear or non-linear relationship with our independent variable, which could allow us to refine our model by using residual as a diagnostic tool. We can then use the Likelihood Ratio Test to confirm whether we can increase the fit of our model by adding more covariates(complexity) to our model.
...
Based on the above Lurking plot, we can see that the all Forest,HFI,Dist_Water have some relation with our predict variable. So we would fit them individually to our model and using Partial Residual to see weather if their relation is linear or Quadratic and then use Likelihood Ratio Test to see if our propose model(more complex model) are better at predicting then our original model(simpler model).
...
```
Looks like Dist_Water have a non-linear relationship with our trend. We then can increase the complexity of our covariate by using a high order polynomials to see will out fitted line follow closely with out observed line:
```{r echo=FALSE, warning=FALSE}
...
```
Visually, our model fitting have improved after adding higher polynomials for both Elevation and Dist_Water. Now we want to run through the Likelihood ratio test to see if this more complex model are in fact increasing the fit of our data compared to our original model. The null-hypothesis of this test is the simple model(reduced model), if the p-value of the test is lower then our significant value(0.05) mean that our more complex model are a better fit of our data. suggesting we should reject the simple model and favor the more complex model. 
...
From our Likelihood Ratio Test, we have achieved a low p-value of 2.2e-16, signify extra complexity is wanted. so we will adopt our new model.


Through the same model validation steps, we want to check if add Forest will improve the fitting of our model.After adding Forest into our model and add increase the complexity of our covariant to improve our fit, we have achieve the following result for our Likelihood Ratio Test result:
```{r echo=FALSE, warning=FALSE}
...
```
From the above Likelihood Ratio Test on the model that have include Forest, we have achieved a low p-value of 2.2e-16, signify extra complexity is wanted. so we will adopt our new model.


Through the same model validation steps to check if add HFI will improve the fitting of our model.After adding HFI into our model and add increase the complexity of our covariant to improve our fit, we have achieve the following result for our Likelihood Ratio Test result:
```{r echo=FALSE, warning=FALSE}
...
From the above Likelihood Ratio Test on the model that have include HFI, we have achieved a low p-value of 2.2e-16, signify extra complexity is wanted. so we will adopt our new model.
...
evidence of autocorrelation among the fungi can hardly be observed during the rest of the higher r value range.
