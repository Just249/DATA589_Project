...
Poisson Model allow us to define a framework for modeling point process, this allow us to treat the number of count in any given cell as poission distributed random variable. and after getting the model we can make better inference about the system and make prediction. In order for us to come up a model that could describe our model well, we first use Lurking Variable plot to gain some insight on whether or not we should include the covariate that were missing from our original model. After know some potential covariates that could improve our model, we then use Partial Residual Plot to understand what type of relationship is our missing covariate with our independent variable, which could allow us to further refine our model by using residual as a diagnostic tool. Lastly, we use Likelihood Ratio Test to confirm whether the new model can actually increase the fit of our model by adding more covariates(complexity) to our model. We then iterate this modeling fitting and validation process till we could find the right model for our data.
...
Form the above model, we can see that all of our features are significant in our model fitting, this indicated that Elevation, HFI, Dist_Water and Forest are a important explanatory factor of the intensity of Ascomycota. Indicated by the quadrat.test(), there is a significant deviation from our model's prediction, so there is still room for improvement in the future, which could achieve by adding more covariates and increase the complexity of existing covariates. Although the model is not yet perfect, it is progressively having improvement after rounds of variables selection process, considering the fact that we are predicting the locations of one species of fungi in a biodiverse continent based only on Elevation and Forest, and have no information on all of the many other factors that would significantly influence fungi growth (e.g. humidity, moisture level, temperature, pH value, oxygen content, etc. )
